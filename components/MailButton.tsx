...
export function MailButton({
  ...
  return (
    <a
      href={`mailto:${email}`}
      className={`group relative inline-flex max-w-full items-center justify-center gap-1.5 overflow-hidden rounded-full border-[3px] border-on-accent bg-accent px-[clamp(10px,3.2vw,34px)] py-[clamp(11px,2.2vw,15px)] text-[clamp(11px,3.4vw,19px)] font-bold whitespace-nowrap text-on-accent [box-shadow:5px_5px_0_var(--color-on-accent)] [transition:translate_0.3s_cubic-bezier(0.34,1.56,0.64,1),box-shadow_0.3s_ease,background-color_0.3s_ease] hover:translate-x-[2px] hover:translate-y-[2px] hover:bg-accent-hover hover:text-on-accent hover:[box-shadow:2px_2px_0_var(--color-on-accent)] ${className}`}
    >
